...
export class EmsEspClient {
  ...
  public async setThermostatText(xcmd: string, xtext: string) {
    await this.post("api/thermostat", { cmd: xcmd , data: xtext });
  }
  ...
  public async setBoilerText(xcmd: string, xtext: string) {
    await this.post("api/boiler", { cmd: xcmd , data: xtext });
  }
